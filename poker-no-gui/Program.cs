...
public class Poker {
    ...
    static void Main(string[] args) {
        ...
        Hand? testHand = GetHand(new List<Card>() { new Card(Rank.Queen, Suit.Spades), new Card(Rank.Jack, Suit.Hearts), new Card(Rank.Ace, Suit.Diamonds), new Card(Rank.King, Suit.Spades), new Card(Rank.Ten, Suit.Hearts), new Card(Rank.Ace, Suit.Spades) });
        Console.WriteLine($"{testHand.Name}; {string.Join(", ", testHand.Cards)};");
    }
    static Hand GetHand(List<Card> cards) {
        ...
        static Hand? getBestPair(List<Card> checkCards) {
            for (int i = 0; i < checkCards.Count - 1; i++) {
                if (checkCards[i].rank == checkCards[i + 1].rank) {
                    ...
                    return new Hand(Hands.OnePair, pair, kickers);
                }
            }
            return null;
        }
        static Hand getBestHighCard(List<Card> checkCards) {
            ...
            if (checkCards.Count > 1)
                kickers = checkCards.GetRange(1, checkCards.Count >= 5 ? 4 : checkCards.Count - 1);
            return new Hand(Hands.HighCard, new List<Card>() { checkCards[0] }, kickers);
        }
    }
